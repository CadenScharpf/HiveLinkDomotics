...
import { INewHome, ISessionUser, PrismaClient, user_home } from "hive-link-common";
...
async function getAll(_: Request, res: Response) {
  ...
  const user = await prisma.user.findUnique({
    where: { id: sUser.id },
    include: { user_home: true },
  });

  return res.status(HttpStatusCodes.OK).json({ homes: user?.user_home });
}

async function newHome(req: IReq<INewHome>, res: Response<{ home: user_home }>) {
