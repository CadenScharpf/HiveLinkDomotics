...
import { ISessionUser, IUpdateHome, PrismaClient, UserRoles } from "hive-link-common";

const prisma = new PrismaClient();
async function getOne(_: IReq, res: Response) {
  try {
    const sUser = await SessionUtil.getSessionData<ISessionUser>(_) as ISessionUser;
    if (!sUser) {
      return res
...
console.log("params", _.params)
    ...
    if(!home) {
      return res.status(HttpStatusCodes.NOT_FOUND).json({ error: "Home not found" });
    }
    if (home.user_id !== sUser.id && sUser.role !== UserRoles.Admin) {
      return res.status(HttpStatusCodes.UNAUTHORIZED).json({ error: "User not authorized to perform this action" });
    }
...
async function updateOne(req: IReq<IUpdateHome>, res: IRes) {
  try {
    const sUser = await SessionUtil.getSessionData<ISessionUser>(req) as ISessionUser;
    if (!sUser) {
      return res
        .status(HttpStatusCodes.UNAUTHORIZED)
        .json({ error: "User not authorized to perform this action" });
